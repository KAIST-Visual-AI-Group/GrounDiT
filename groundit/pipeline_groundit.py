...
# Modified by Taehoon Yoon (taehoon@kaist.ac.kr) & Phillip Lee (phillip0701@kaist.ac.kr)
import html
import copy
import inspect
import re
...
from groundit.global_update_functions import *
from groundit.local_update_functions import *
# ================== GrounDiT ================== #
...
class PixArtAlphaPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        phrases = None,
        phrases_idx = None,
        # (2) Global Update Arguments
        loss_scale = 10,
        loss_threshold = 0.00001,
        gradient_weight = 5.0,
        global_update_max_iter_per_step = 3,
        # (3) Local Update Arguments
        object_image_latents_list = None,
        object_image_hw_in_pixel_space = None,
        all_bbox_coord_in_latent_space = None,
        # ================== GrounDiT ================== #
        **kwargs,
       ...
       # 4. Prepare timesteps
        ...
        # Prepare micro-conditions for object images 
        # & Prepare schedulers for object images and local patches
        local_patch_scheduler_list = []
        obj_img_scheduler_list = []
        obj_img_added_cond_kwrags_list = []

        for object_image_hw_list in object_image_hw_in_pixel_space:  
            obj_img_added_cond_kwargs = []
            obj_img_scheduler = []
            local_patch_scheduler = []

            for obj_img_h, obj_img_w in object_image_hw_list:
                # Add scheduler for each object image
                obj_img_scheduler.append(copy.deepcopy(self.scheduler))

                # Add scheduler for each local patch
                local_patch_scheduler.append(copy.deepcopy(self.scheduler))

                if self.transformer.config.sample_size == 128:
                    # Actual resolution of the object image
                    resolution_obj_img = torch.tensor([obj_img_h, obj_img_w])
                    resolution_obj_img = resolution_obj_img.repeat(batch_size * num_images_per_prompt, 1)
                    resolution_obj_img = resolution_obj_img.to(dtype=prompt_embeds.dtype, device=device)

                    # Aspect ratio of the object image
                    aspect_ratio_obj_img = torch.tensor([float(obj_img_h / obj_img_w)])
                    aspect_ratio_obj_img = aspect_ratio_obj_img.repeat(batch_size * num_images_per_prompt, 1)
                    aspect_ratio_obj_img = aspect_ratio_obj_img.to(dtype=prompt_embeds.dtype, device=device)

                    if do_classifier_free_guidance:
                        resolution_obj_img = torch.cat([resolution_obj_img, resolution_obj_img], dim=0)
                        aspect_ratio_obj_img = torch.cat([aspect_ratio_obj_img, aspect_ratio_obj_img], dim=0)

                    added_cond_kwargs_obj = {
                        "resolution": resolution_obj_img, 
                        "aspect_ratio": aspect_ratio_obj_img
                    }
                    obj_img_added_cond_kwargs.append(added_cond_kwargs_obj)
                else:
                    obj_img_added_cond_kwargs.append({"resolution": None, "aspect_ratio": None})

            # Append to the list (object image, local patch)
            obj_img_scheduler_list.append(obj_img_scheduler)
            local_patch_scheduler_list.append(local_patch_scheduler)
            obj_img_added_cond_kwrags_list.append(obj_img_added_cond_kwargs)
        ...
        aggregated_grounding_loss_args = {
        ...
        # Global Update
        GlobalUpdate = partial(
            GlobalUpdateFunc, 
            denoiser_args=denoiser_args, 
            loss_args=aggregated_grounding_loss_args
        )
        # ================== GrounDiT ================== #
        ...
        with torch.enable_grad():
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    # =========== Global Update (See Sec. 5.1 in the paper) =========== #
                    glboal_update_iter_cnt = 0
                    while (glboal_update_iter_cnt < global_update_max_iter_per_step and i < groundit_denoising_step):
                        latents, loss = GlobalUpdate(self, latents, t, i)
                        if loss <= loss_threshold:
                            break
                        glboal_update_iter_cnt += 1
                    # =========== Global Update (See Sec. 5.1 in the paper) =========== #

                    # =========== Local Update (See Sec. 5.3 in the paper) =========== #
                    with torch.no_grad():
                        # =========== Object Branch =========== #
                        misc_args = {
                        ...
                        if i < groundit_denoising_step:
                            # Object Branch Arguments
                            object_image_args = {
                                "object_image_latents_list": object_image_latents_list, 
                                "obj_img_scheduler_list": obj_img_scheduler_list, 
                                "obj_img_added_cond_kwrags_list": obj_img_added_cond_kwrags_list
                            }
                            local_patch_args = {
                                "all_bbox_coord_in_latent_space": all_bbox_coord_in_latent_space, 
                                "local_patch_scheduler_list": local_patch_scheduler_list
                            }
                            phrase_args = {
                                "phrase_embeds_list": phrase_embeds_list, 
                                "phrase_attention_mask_list": phrase_attention_mask_list
                            }

                            # Object Branch Joint Denoising
                            local_patch_latent_list = ObjectBranch(
                                object_image_args, 
                                local_patch_args, 
                                phrase_args, 
                                misc_args, 
                        ...
                        if i < groundit_denoising_step:
                            NoisyPatchTransplantation(
                                latents, 
                                local_patch_latent_list, 
