...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        ################################################################################################################
        #                                                                                                              #
        # In GrounDiT, we use AttnProcessor() instead of AttnProcessor2_0() to extract the attention maps.             #
        #                                                                                                              #                               
        ################################################################################################################
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if return_attn_maps:
            ...
            attention_probs = torch.mean(attention_probs, axis=1)

            # [WARNING] Limiting the the number of tokens to 120 should not be a problem for most cases, since currently tokenizer's max_length is set to 120.
            MAX_NUM_TOKENS = 120  
            attention_probs = attention_probs[:, :, :MAX_NUM_TOKENS]     
        if return_attn_maps:
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        assert False, "This method should not be called. Please use the `AttnProcessor` instead."
        if len(args) > 0 or kwargs.get("scale", None) is not None:
            deprecation_message = "The `scale` argument is deprecated and will be ignored. Please remove it, as passing it will raise an error in the future. `scale` should directly be passed while calling the underlying pipeline component i.e., via `cross_attention_kwargs`."
            deprecate("scale", "1.0.0", deprecation_message)
